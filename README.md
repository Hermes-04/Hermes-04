
# Hi, I'm Hayesh Raghavan 👋

### CSE Student | Aspiring Business Analyst

I'm a Computer Science Engineering student interested in **Business Analytics, Data, and Technology**.

I enjoy working with data, solving problems, and learning how technology can be used to make better business decisions.

---

## 🛠️ Skills

- Python
- SQL
- Excel
- Pandas
- Data Cleaning & Analysis
- Matplotlib

---

## 📊 Currently Learning

- Advanced SQL
- Statistics & Probability
- Data Visualization
- Business Analytics
- Machine Learning

---


## 🚀 Projects

### 📊 Sales Data Analysis

Exploratory sales data analysis using Python, Pandas, and Matplotlib.

**What I analyzed:**
- Sales and profit KPIs
- Product performance
- Regional performance
- Monthly sales trends
- Sales efficiency by region

**Key findings:**
- ₹44.55 lakh total revenue
- ₹10.81 lakh total profit
- 24.27% profit margin
- Laptops contributed 75.88% of total revenue
- North was the highest-revenue region
- March recorded the highest monthly sales

🔗 [View Project](https://github.com/Hermes-04/sales-data-analysis)

## 🎯 Goal

To build strong skills in **Business Analytics** and work on real-world data-driven problems.

---

## 📫 Connect With Me
- Email: the.hermes.28@gmail.com
